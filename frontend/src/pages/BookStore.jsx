...
import { getAllBooks, searchBooks, filterBooksByType, setCurrentPage, clearSearchResults, clearFilterResults } from '../redux/slices/book.slice';
import SidebarHeader from '../components/SidebarHeader';

const BookStore = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { pages, searchResults, filterResults, loading, error, totalBooks, totalPages, currentPage, hasFetchedInitial } = useSelector((state) => state.books);
  ...
  useEffect(() => {
    if (!hasFetchedInitial) {
      dispatch(getAllBooks({ page: 1, limit }));
    }
  ...
  useEffect(() => {
    if (debouncedQuery) {
      dispatch(searchBooks({ query: debouncedQuery, page: 1, limit }));
    } else {
  ...
  useEffect(() => {
    if (filterType) {
      dispatch(filterBooksByType({ type: filterType, page: 1, limit }));
    } else {
  ...
  const handleNextPage = () => {
    if (currentPage < totalPages) {
      ...
      if (debouncedQuery) {
        dispatch(searchBooks({ query: debouncedQuery, page: nextPage, limit }));
      } else if (filterType) {
        dispatch(filterBooksByType({ type: filterType, page: nextPage, limit }));
      } else {
        dispatch(getAllBooks({ page: nextPage, limit }));
      }
  ...
  const displayedBooks = debouncedQuery
    ? searchResults.books
    : filterType
    ? filterResults.books
    : pages[currentPage] || [];
  ...
  // Book types for filter dropdown
  const bookTypes = ['', 'Fiction', 'Non-Fiction', 'Mystery', 'Sci-Fi', 'Fantasy'];
  ...
  return (
    <SidebarHeader>
      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {(loading || isTyping) ? (
        ...
        ) : displayedBooks.length === 0 && !loading && !isTyping ? (
          <div className="flex items-center justify-center min-h-[50vh]">
            <p className="text-orange-500 text-xl">No books available</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {displayedBooks.map((book) => (
              ...
              >
                <img
                  src={book.bookImage[0]?.secure_url || 'https://via.placeholder.com/150'}
                  alt={book.bookName}
                  className="w-full h-48 object-cover"
                  onError={(e) => (e.target.src = 'https://via.placeholder.com/150')}
                />
                <div className="p-4">
                  <h3 className="text-lg font-semibold text-orange-500 truncate">{book.bookName}</h3>
        ...
        {/* Pagination */}
        {!loading && !isTyping && displayedBooks.length > 0 && (
